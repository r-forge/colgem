#~ based on pseudolik...ode3, this version uses pseudolikelihood...1, see new objfun
#~ based on pseudolik...ode2, this version keeps alpha fixed, uses stats4::mle
require(dfoptim)
require(ape)
require(deSolve)
#~ require(stats4)
...
if(TRUE)
{
	...
	pltime <- system.time( { ll <- calculate.coalescent.log.pseudolikelihood.1(sampleTime, bdt, heights_entanglements, FGY=FGY, integrationMethod = 'rk4', discretizeRates=TRUE, fgyResolution = 200) } )
	print(ll)
	print(pltime)
}
...
#~ 		for (n in INFECTEDNAMES){ lines(times0, Ys[,n], lty='solid' ) }
	...
	.obj5 <- function(beta0, beta1, beta2, I0_0)
	{ #pseudolikelihood (entanglements)
		theta1 <- parms_truth
		theta1[ESTNAMES] <- exp(c(beta0, beta1, beta2, I0_0))
		print(unname( unlist(theta1[ESTNAMES])) )
		FGY <- solve.ode.model.set.fgy(theta1)
		mll <- -calculate.coalescent.log.pseudolikelihood.1(sampleTime, bdt, heights_entanglements, FGY=FGY, integrationMethod = 'rk4', discretizeRates=TRUE, fgyResolution = 200)
		#round(mll, 3)
		mll
	}
	...
	fits <- lapply( 1:NFITS, function(i)
	{
		...
		#  error = function(e) browser() )
		tryCatch( {
		  fit <- mle2(of, start = as.list(theta), method='BFGS', skip.hessian=TRUE, control=list(trace=6,maxit=300,  hessian=FALSE, reltol=1e-06) )
		}, error = function(e) NULL)
		#fit <- mle2(of, start = as.list(theta), method='Nelder-Mead', skip.hessian=TRUE, control=list(trace=6,maxit=300,  hessian=FALSE) )
		# parscale= c(1, .1, .1, 1),
	})
	print(paste(date(), 'done with fits iter 1'))
	fits_bak <- fits
	for (i in NFITS:1) { if (is.null( fits[[i]] )) { fits[[i]] <- NULL } }
	bestfit_i <- which.min( sapply(1:NFITS, function(i) 
	  tryCatch( {-logLik(fits[[i]]) }, error = function(e) Inf )
	 ) )
	fit <- fits[[bestfit_i]]
	if (attr(fit, 'details')$convergence!=0)
	{
		fit <- mle2(of, start = as.list(coef(fit)), method='BFGS', skip.hessian=TRUE, control=list(trace=6,maxit=300,  hessian=FALSE, reltol=1e-06) )
		#fit = fit_ml <- mle2(of, start = as.list(coef(fit)), method='Nelder-Mead',reltol=1e-06,control=list(trace=6,maxit=300,  hessian=FALSE) )
